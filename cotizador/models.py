...
class TransporteApp(models.Model):
    ...
    tarifa_base = models.DecimalField(max_digits=8, decimal_places=2, default=3000)
    costo_por_km = models.DecimalField(max_digits=8, decimal_places=2, default=800)
    max_multiplicador = models.DecimalField(max_digits=3, decimal_places=2, default=1.50)
...
class CotizacionTraslado(models.Model):
    ...
    def __str__(self):
        return f"Cotización #{self.pk} - {self.origen_nombre} a {self.destino_nombre}"
